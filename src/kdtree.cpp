...
BoundingBox bounding_box(const pointVec& cluster) {
    BoundingBox box;
    box.x_min = box.y_min = box.z_min =  std::numeric_limits<double>::max();
    box.x_max = box.y_max = box.z_max = -std::numeric_limits<double>::max();

    for (const auto& pt : cluster) {
        // Expecting pt.size() >= 3
        if (pt[0] < box.x_min) box.x_min = pt[0];
        if (pt[1] < box.y_min) box.y_min = pt[1];
        if (pt[2] < box.z_min) box.z_min = pt[2];

        if (pt[0] > box.x_max) box.x_max = pt[0];
        if (pt[1] > box.y_max) box.y_max = pt[1];
        if (pt[2] > box.z_max) box.z_max = pt[2];
    }
    return box;
}
